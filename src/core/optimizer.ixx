...
namespace erdo::optimizer
{
    ...
    struct V2 : OptimizerBase<target>
    {
        ...
        static VariedAttributes get_optimized_varied_attributes(const NonscalingAttributes& nonscaling_attributes)
        {
            auto optimized_varied_attributes = default_varied_attributes;
            for (auto&& [nonscaling_attribute, optimized_varied_attribute] : std::views::zip(
                nonscaling_attributes,
                optimized_varied_attributes | std::views::drop(irrelevant_attribute_count)
            ))
                optimized_varied_attribute = !nonscaling_attribute;

            return optimized_varied_attributes;
        }
                ...
                if (inserted)
                {
                    if (optimize_starting_class)
                    {
                        optimized_stat_variations = starting_class::get_stat_variations(
                            ...
                            starting_class::get_min_stats(min_attr_lvls),
                            max_attr_lvls,
                            get_optimized_varied_attributes(nonscaling_attributes)
                        );
                    }
                    else
                    {
